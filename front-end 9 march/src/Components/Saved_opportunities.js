...
export default function Saved_opportunities() {
  ...
  const [savedOppoData, setSavedOppoData] = useState([]);

  useEffect(() => {
    async function fetchData() {
      let opportunityAllData2 = await fetch(
      ...
      const saveoppoArray = [];
      user.savedOpportunities.forEach((element) => {
        saveoppoArray.push(arrayrev2.find((each) => each._id === element));
      });
      console.log(saveoppoArray);
      setSavedOppoData(saveoppoArray.reverse());
    }
    fetchData();
  }, []);

  async function handleDeleteSavedOppo(id) {
    let result = await fetch(
      `http://localhost:4000/api/users/opportunity/${id}/${userDetails.email}`,
      {
        method: "DELETE",
        headers: {
            ...
            <div className="card artist-card">
              <div className="card-body">
                ...
                <div className="row patron-search-artist-body-footer">
                  ...
                  <div>
                    {/* <button onClick={handleDeleteSavedOppo(`${item._id}`)}>
                      Delete
                    </button>*/}
                  </div>
                  <div className="col-auto artist-details">
                    <Link to={`/Artist_viewdetailopp/${item._id}`}>
                      View details
                      <img
